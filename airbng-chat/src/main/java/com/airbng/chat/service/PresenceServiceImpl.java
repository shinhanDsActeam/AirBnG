...
//    private String globalKey() { return "presence:online:users"; }
    ...
    private static final String KEY_LAST_SEEN  = "presence:lastseen";
    private static final int SESSION_TTL_SEC = 90; // 세션키 TTL
    ...
    private void setLastSeen(long userId, long epochSec) {
        redis.opsForHash().put(KEY_LAST_SEEN, String.valueOf(userId), String.valueOf(epochSec));
    }
    ...
    private void upsertBucketIfChanged(long userId, long bucket) {
        ...
        if (bucket > curBucket) {
            ...
            setLastSeen(userId, bucket); // ← 최근 활동도 함께 기록
        }
...
//        if (ttlSeconds <= 0) ttlSeconds = 60 * 60; // 안전한 디폴트(1h)
        ...
        // 2) 세션키 저장(유저 id 값, TTL)
        redis.opsForValue().set(sessionKey(sessionId), String.valueOf(userId), Duration.ofSeconds(SESSION_TTL_SEC));
        // 3) 최근활동 버킷
    ...
    public void touch(long userId) {
        ...
        // 이 부분이 중요: 현재 사용자의 모든 세션 TTL을 연장
        var sids = redis.opsForSet().members(userKey(userId));
        if (sids != null) {
            for (String sid : sids) {
                redis.expire(sessionKey(sid), Duration.ofSeconds(SESSION_TTL_SEC));
            }
        }
    }
    ...
    public void offline(long userId, String sessionId) {
        ...
        // 오프라인 처리 시각도 기록
        setLastSeen(userId, System.currentTimeMillis() / 1000L);
    ...
    public void offlineBySessionId(String sessionId) {
        ...
        try {
            ...
            redis.opsForSet().remove(userKey(userId), sessionId);
            setLastSeen(userId, System.currentTimeMillis() / 1000L); // 기록
            ensureUserClean(userId);
    ...
    @Override
    public Long lastSeenAt(long userId) {
        Object v = redis.opsForHash().get(KEY_LAST_SEEN, String.valueOf(userId));
        if (v == null) return null;
        try { return Long.parseLong(String.valueOf(v)); } catch (NumberFormatException e) { return null; }
    }

    @Override
    public Map<Long, Long> lastSeenBulk(List<Long> userIds) {
        if (userIds == null || userIds.isEmpty()) return Map.of();
        var keys = userIds.stream().map(String::valueOf).toList();
        var vals = redis.opsForHash().multiGet(KEY_LAST_SEEN, new ArrayList<>(keys));
        Map<Long, Long> out = new HashMap<>();
        for (int i = 0; i < keys.size(); i++) {
            Object v = (vals != null && i < vals.size()) ? vals.get(i) : null;
            if (v == null) continue;
            try { out.put(Long.valueOf(keys.get(i)), Long.parseLong(String.valueOf(v))); }
            catch (NumberFormatException ignore) {}
        }
        return out;
    }
