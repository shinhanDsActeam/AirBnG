package com.airbng.chat.interceptor;

import com.airbng.api.security.PrincipalApi;
import com.airbng.chat.security.auth.StompAuthToken;
...
public class StompAuthChannelInterceptor implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if (StompCommand.CONNECT.equals(acc.getCommand())) {
            ...
            if (token == null) throw new BadCredentialsException("Missing Authorization");
            ...
            var principal = principalApi.requireById(userId); // AirbngPrincipal 구현체
            Authentication auth = new StompAuthToken(principal);

            acc.setUser(auth);
            acc.getSessionAttributes().put(SESSION_AUTH_KEY, auth);

            // 온라인 등록은 SessionConnectedEvent 리스너(ChatPresenceListener)에서 처리
        }
